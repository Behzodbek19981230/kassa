...
export default function ClientPage() {
	...
	const columns = [
		columnHelper.accessor('fio', { header: 'F.I.Sh', meta: { align: 'left' } }),
		...
		columnHelper.display({
			...
			cell: ({ row }) => (
				<div className='flex flex-wrap gap-1'>
					{Boolean(row.original.is_worker) && <Badge variant='info'>Ishchi</Badge>}
					{Boolean(row.original.is_partner) && <Badge variant='theme'>Hamkor</Badge>}
				</div>
			),
		}),
		columnHelper.display({
