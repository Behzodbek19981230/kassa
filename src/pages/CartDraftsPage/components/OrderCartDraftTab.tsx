...
export default function OrderCartDraftTab({ onRefetchReady }: OrderCartDraftTabProps) {
	...
	return (
		<>
			...
			{!isLoading && !isError && groups.length === 0 && <p className='text-center'>Karzinkalar bo'sh</p>}
			{!isLoading &&
				!isError &&
				groups.map((group) => {
					...
					return (
						<div key={group.client_id} className='mb-6 overflow-hidden rounded-[3px] border border-ca-border last:mb-0'>
							<div className='flex flex-wrap items-center justify-between gap-2 bg-ca-silver px-3 py-2'>
								<span className='text-xs font-semibold text-ca-heading'>
									{group.client_fio}{' '}
									<span className='font-normal text-ca-text'>({formatDateTime(group.created_time)})</span>
								</span>
								<Button
									type='button'
										...
										{group.items.map((item, index) => {
											...
											return (
												<TableRow key={item.id} className='bg-red-50'>
													...
													<TableCell>{item.warehouse_detail?.product_category_name ?? '-'}</TableCell>
													...
													<TableCell className='font-semibold'>{formatNumber(totalPrice, 2)} $</TableCell>
													<TableCell>
														<Button
															type='button'
															{...buttonProps(<FaTrash />, 'danger', 'icon')}
			...
			<Modal open={Boolean(groupToDelete)} onOpenChange={(open) => !open && setGroupToDelete(null)}>
				<ModalContent>
					...
					<ModalBody>
						<p>
							<span className='font-semibold'>{groupToDelete?.clientFio}</span> mijozning karzinkasidagi barcha
