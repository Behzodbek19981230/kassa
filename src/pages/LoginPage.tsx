    ...
    <div className="flex min-h-screen items-center justify-center bg-ca-body px-4 py-16">
      <div className="w-full max-w-[450px]">
        <div className="relative mb-5 px-2.5">
          ...
          />
          <span className="align-middle text-[28px] font-light text-ca-heading">Color Admin</span>
          <small className="block pl-[38px] text-sm font-light text-ca-text">
        ...
        <div className="rounded-[3px] border border-ca-border bg-white px-10 py-8 shadow-[0_10px_30px_rgba(15,23,42,0.08)]">
          ...
          <div className="mb-5 rounded border border-ca-border bg-ca-silver px-3 py-2 text-[11px] text-ca-text">
            Demo login — email <strong className="text-ca-heading">{DEFAULT_CREDENTIALS.email}</strong>, password{' '}
            <strong className="text-ca-heading">{DEFAULT_CREDENTIALS.password}</strong>
          </div>
          ...
          <form onSubmit={onSubmit} noValidate>
            <div className="mb-5">
              <input
                ...
                {...register('email')}
              />
              {errors.email && <p className="mt-1 text-[11px] text-ca-red">{errors.email.message}</p>}
            </div>
            <div className="mb-5">
              <input
                type="password"
                ...
                {...register('password')}
              />
              {errors.password && <p className="mt-1 text-[11px] text-ca-red">{errors.password.message}</p>}
            </div>
